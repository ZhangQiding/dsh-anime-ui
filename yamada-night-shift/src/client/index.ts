/**
 * Deep-sea yamada atelier skin. The client entry keeps the bare two-character
...
import {
  YAMADA_NIGHT_SHIFT_MAID_LEFT,
  YAMADA_NIGHT_SHIFT_MAID_RIGHT,
...
const SKIN_SYSTEM_CHROME_COLOR = '#090b10'
const SKIN_ICON = `data:image/svg+xml,${encodeURIComponent(`
  <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 64 64">
    <rect width="64" height="64" rx="16" fill="#090b10"/>
    <circle cx="32" cy="30" r="18" fill="#7d293a"/>
    <path d="M38 12a18 18 0 1 0 10 32 20 20 0 1 1-10-32Z" fill="#f0d6d9"/>
    <circle cx="48" cy="14" r="3" fill="#d07886"/>
  </svg>
`)}`
...
const PROJECTED_STATE_ATTRIBUTES = {
  activeChat: 'data-yamada-chat-active',
  activeConversation: 'data-yamada-conversation-active',
  betterSidebarOpen: 'data-yamada-better-sidebar-open',
  cordisPanelOpen: 'data-yamada-cordis-panel-open',
  settingsOpen: 'data-yamada-settings-open',
  workspace: 'data-yamada-workspace',
} as const
...
const BACKDROP_PROPERTIES = [
  ...
  'background-repeat',
  '--yamada-sidebar-width',
  '--yamada-top-trim-art',
  '--yamada-bottom-trim-art',
  '--yamada-bottom-crest-art',
  '--yamada-bow-art',
  '--yamada-new-session-art',
  '--yamada-sidebar-swag-art',
  '--yamada-sidebar-corner-art',
  '--yamada-composer-frame-art',
  '--yamada-settings-frame-art',
  '--yamada-workspace-crest-art',
  '--yamada-workspace-ribbon-art',
] as const
...
function createCharacterStage(): HTMLDivElement {
  ...
  left.dataset.yamadaCharacter = 'left'
  left.alt = ''
  left.src = YAMADA_NIGHT_SHIFT_MAID_LEFT

  const right = document.createElement('img')
  right.dataset.yamadaCharacter = 'right'
  right.alt = ''
  right.src = YAMADA_NIGHT_SHIFT_MAID_RIGHT
...
function decorateTitlebarBrand(ownedNodes: Set<Element>): void {
  ...
  brand.textContent = 'YAMADA / NIGHT SHIFT'
...
function decorateSidebar(ownedNodes: Set<Element>, decoratedElements: Set<HTMLElement>): void {
  ...
  sidebar.querySelectorAll<HTMLElement>('[data-yamada-sidebar-footer]').forEach((element) => {
    delete element.dataset.yamadaSidebarFooter
  })
  ...
  if (settingsSlot) {
    ...
    while (footer && footer !== sidebar) {
      if (footer.querySelector("[data-slot='sidebar.footer.action']")) {
        footer.dataset.yamadaSidebarFooter = ''
  ...
  if (!sidebarRoot.querySelector("[data-skin-chrome='sidebar-mascot']")) {
    ...
    mascot.alt = ''
    mascot.src = YAMADA_NIGHT_SHIFT_MAID_RIGHT
...
function decorateWorkspaceTree(decoratedElements: Set<HTMLElement>): void {
  ...
  sidebar.querySelectorAll<HTMLElement>(
    '[data-yamada-workspace-group], [data-yamada-workspace-row], [data-yamada-workspace-active], [data-yamada-session-row], [data-yamada-session-flat], [data-yamada-session-first], [data-yamada-session-last]',
  ).forEach((element) => {
    delete element.dataset.yamadaWorkspaceGroup
    delete element.dataset.yamadaWorkspaceRow
    delete element.dataset.yamadaWorkspaceActive
    delete element.dataset.yamadaSessionRow
    delete element.dataset.yamadaSessionFlat
    delete element.dataset.yamadaSessionFirst
    delete element.dataset.yamadaSessionLast
  })
  ...
  sidebar.querySelectorAll<HTMLElement>("[role='tree']").forEach((tree) => {
    ...
    if (tree.matches("[class*='flatList']") && !rows.some(row => row.hasAttribute('aria-expanded'))) {
      rows.filter(row => row.hasAttribute('aria-selected')).forEach((sessionRow) => {
        sessionRow.dataset.yamadaSessionRow = ''
        sessionRow.dataset.yamadaSessionFlat = ''
    ...
    const decorateGroup = (): void => {
      ...
      workspaceRow.dataset.yamadaWorkspaceRow = ''
      decoratedElements.add(workspaceRow)
      if (workspaceRow.parentElement) {
        workspaceRow.parentElement.dataset.yamadaWorkspaceGroup = ''
      ...
      if (sessionRows[0]) sessionRows[0].dataset.yamadaSessionFirst = ''
      if (sessionRows.at(-1)) sessionRows.at(-1)!.dataset.yamadaSessionLast = ''
      ...
      if (containsCurrent) workspaceRow.dataset.yamadaWorkspaceActive = ''
    }
...
export function apply(ctx: Context): void {
  ...
  const skinScopeLease = createBodyAttributeLease(body, 'data-dsh-yamada-night-shift')
  const viewportResizeLease = createBodyAttributeLease(body, 'data-yamada-viewport-resizing')
  const lowPowerLease = createBodyAttributeLease(body, 'data-yamada-low-power')
  ...
  ctx.effect(() => () => {
    ...
    delete body.dataset.yamadaSidebarSize
  ...
  }, 'ui-skin-yamada-night-shift: layered background and ornament')

  skinScopeLease.acquire()
  ...
  widthSheet.sheet!.insertRule('body { --yamada-sidebar-width: 280px; --yamada-sidebar-swag-height: 72.1px; --yamada-sidebar-mascot-width: 229.6px; --yamada-titlebar-height: 0px; }')
  ...
  appendRule('body[data-dsh-yamada-night-shift] [class*=\"frame\"][data-wco] { grid-template-rows: env(titlebar-area-height, 40px) 1fr; }')
  appendRule('body[data-dsh-yamada-night-shift] [class*=\"frame\"][data-desktop] { grid-template-rows: 32px 1fr; }')
  appendRule('body[data-dsh-yamada-night-shift] [class*=\"frame\"] [class*=\"handle\"] { top: var(--yamada-titlebar-height, 0px); }')
  ...
  syncTitlebarHeight = (): void => {
    ...
    if (columns !== null) {
      ...
      if (top > 0) {
        widthRule.style.setProperty('--yamada-titlebar-height', `${top}px`)
        return
      }
    }
    // Desktop shell: fixed 32px row (columns not laid out yet).
    if (document.querySelector("[class*='frame'][data-desktop]") !== null) {
      widthRule.style.setProperty('--yamada-titlebar-height', '32px')
      return
    }
    widthRule.style.setProperty('--yamada-titlebar-height', '0px')
  }
  ...
  const applySidebarWidth = (width: number): void => {
    ...
    widthRule.style.setProperty('--yamada-sidebar-width', roundPx(width))
    widthRule.style.setProperty('--yamada-sidebar-swag-height', roundPx(Math.min(94, Math.max(54, width * 0.2575))))
    widthRule.style.setProperty('--yamada-sidebar-mascot-width', roundPx(Math.min(320, width * 0.82)))
    body.dataset.yamadaSidebarSize = width <= 120 ? 'rail' : width <= 220 ? 'narrow' : 'wide'
    if (width <= 104) body.dataset.yamadaSidebarCompact = ''
    else delete body.dataset.yamadaSidebarCompact
  }

  const clearSidebarWidth = (): void => {
    widthRule.style.setProperty('--yamada-sidebar-width', '0px')
    widthRule.style.setProperty('--yamada-sidebar-swag-height', '54px')
    widthRule.style.setProperty('--yamada-sidebar-mascot-width', '0px')
    body.dataset.yamadaSidebarSize = 'rail'
    body.dataset.yamadaSidebarCompact = ''
  }
  ...
  const syncComposerMotion = (): void => {
    ...
    if (composerPhase !== undefined && composerPhase !== next) {
      body.dataset.yamadaComposerMotion = next === 'active' ? 'dock' : 'rise'
      if (composerMotionTimer !== undefined) clearTimeout(composerMotionTimer)
  ...
  const syncSettingsBackdropFrame = (): void => {
    ...
    if (settingsBackdropFrame === undefined) {
      ...
      settingsBackdropFrame.dataset.yamadaSettingsBackdropFrame = ''
